...
// New York: THIRTEEN feeds, one network. The MTA publishes its own from an
// S3 bucket (rrgtfsfeeds.s3.amazonaws.com): the subway, the five borough
// bus feeds of New York City Transit (which all carry the same routes.txt
// and differ only in their trips), the MTA Bus Company, the Long Island Rail
// Road and Metro-North. Nassau County's NICE bus is its own feed, PATH comes
// from Trillium, and NJ Transit's bus and rail feeds — behind a developer
// login on njtransit.com — are read from the MobilityDatabase mirror. The
// New Jersey side is the Hudson–Essex core only: Jersey City, Hoboken and
// Newark (pipeline/scope.mjs → data/scope.json), with the two light rail
// lines and PATH; the NJT commuter rail stays out as a state-scale network.
// Four cfgs: buses navy (NYCT, MTA Bus, NICE and NJ Transit alike — the
// feeds' own colours are not repeated, colour means the MODE here), the two
// light rail lines family red, the subway and PATH in their official line
// colours, and the commuter rail (LIRR, Metro-North, Staten Island Railway)
// in the branch colours the feeds ship, both rail cfgs with the trunk
// treatment — wide ribbon, station discs, always-on names.
// Usage: node pipeline/build.mjs [--all | lines...] [--tram all|A,7,Babylon,HOB-33]
...
// prints (HOB-33, NWK-WTC…), the LIRR by branch name without "Branch", the
// Metro-North lines by name, the Staten Island Railway as SIR, and the two
// light rail lines as HBLR and NLR.
//
// Cut deliberately: the NJT commuter rail (a state-scale network — a single
// Port Jervis train would stretch the frame 120 km north-west); the River
// Line (Camden–Trenton); the LIRR "City Terminal Zone" route (Penn / Grand
// Central / Atlantic Terminal ↔ Jamaica shuttles on track every branch
// already draws); the subway shuttle buses (711); NYC Ferry, the Staten
// Island Ferry and NY Waterway (the engine has no water graph).
const LBL = new Map();

const PATH_KEYS = {
  859: 'HOB-33', 860: 'HOB-WTC', 862: 'NWK-WTC', 861: 'JSQ-33',
  1024: 'JSQ-33 via HOB', 74320: 'NWK-HAR', 77285: 'WTC-33',
};

// the allowlist that IS the New Jersey bus scope — build refuses to guess
...
const S_NJ = new Set(JSON.parse(readFileSync(SCOPE_FILE, 'utf8')).nj);

// Line colours from the feeds: the subway's trunk colours (A/C/E blue, B/D/F/M
// orange, 1/2/3 red…), PATH's four, the LIRR branch colours, Metro-North's
// three (Hudson green, Harlem blue, New Haven red with its branches), the
// SIR blue. Light rail keeps the family red — HBLR and NLR are one system
// each, not lines of a colour code.
const NYC_COLORS = {};
const hex = (c) => (/^[0-9A-F]{6}$/i.test(c || '') ? '#' + c.toUpperCase() : null);
for (const r of await readCsv(join(ROOT, 'data/gtfs-subway/routes.txt'))) {
  const c = hex(r.route_color);
  if (c) NYC_COLORS[r.route_type === '2' ? 'SIR' : r.route_id] = c;
}
for (const r of await readCsv(join(ROOT, 'data/gtfs-path/routes.txt'))) {
  const k = PATH_KEYS[r.route_id], c = hex(r.route_color);
  if (k && c) NYC_COLORS[k] = c;
}
const lirrKey = (r) => {
  const n = (r.route_long_name || '').trim();
  if (n === 'City Terminal Zone') return null;
  return n.replace(/ (Branch|Service)$/, '');
};
for (const r of await readCsv(join(ROOT, 'data/gtfs-lirr/routes.txt'))) {
  const k = lirrKey(r), c = hex(r.route_color);
  if (k && c) NYC_COLORS[k] = c;
}
for (const r of await readCsv(join(ROOT, 'data/gtfs-mnr/routes.txt'))) {
  const k = (r.route_long_name || '').trim(), c = hex(r.route_color);
  if (k && c) NYC_COLORS[k] = c;
}
const isRailTrunk = (l) => l in NYC_COLORS;

// the three subway shuttles print the S the trains carry
for (const k of ['GS', 'FS', 'H']) LBL.set(k, 'S');

// --tram feeds three rail cfgs here, told apart by the key: subway route ids
// and PATH pairs, the LIRR/MNR/SIR names, HBLR/NLR
const SUBWAY_KEYS = new Set(['SIR', '1', '2', '3', '4', '5', '6', '6X', '7', '7X', 'A', 'B', 'C', 'D', 'E', 'F', 'FX',
  'G', 'J', 'Z', 'L', 'M', 'N', 'Q', 'R', 'W', 'GS', 'FS', 'H', ...Object.values(PATH_KEYS)]);
const LRT_KEYS = new Set(['HBLR', 'NLR']);
const subwaySel = tramLines.filter((l) => SUBWAY_KEYS.has(l));
const lrtSel = tramLines.filter((l) => LRT_KEYS.has(l));
const railSel = tramLines.filter((l) => l !== 'all' && !SUBWAY_KEYS.has(l) && !LRT_KEYS.has(l));

// Names as the signs show them. The MTA bus feeds and NJ Transit SHOUT
// ("WILLIS AV/E 138 ST", "MAIN ST AT ADAMS AVE", headsigns included) and
// print the street abbreviations of their pole flags; usName brings a
// fully-uppercase name to title case, keeps the abbreviations the flags use
// (Av, St, Blvd, Pkwy…) and the acronyms that are names (JFK, LGA, NYU), and
// leaves a mixed-case name alone. NJ Transit's headsigns also carry the route
// number and a fare note ("1 JERSEY CITY JOURNAL SQ VIA RIVER TERMINAL-Exact
// Fare") — both go.
const KEEP = new Set(['JFK', 'LGA', 'EWR', 'NYU', 'NY', 'NJ', 'NYC', 'MTA', 'SBS', 'PATH', 'LIRR', 'SIR', 'CUNY',
  'SUNY', 'IND', 'BMT', 'IRT', 'WTC', 'GWB', 'LIC', 'FDR', 'JCMC', 'UPS', 'USPS', 'DMV', 'YMCA', 'VA', 'HS', 'MS', 'PS',
  'IS', 'JHS', 'SI', 'BX', 'UN', 'US', 'II', 'III', 'IV', 'NE', 'NW', 'SE', 'SW', 'N', 'S', 'E', 'W', 'PA', 'GO',
  'NJT', 'HBLR', 'NLR', 'PABT', 'PANYNJ', 'MLK', 'JFK', 'LGA', 'SUNY', 'BQE', 'LIE', 'FDR', 'RR', 'AMTRAK', 'NJPAC', 'UMDNJ', 'NJIT', 'IKEA', 'BJ\'S', 'CVS', 'LI']);
const ABBR = {
  AV: 'Av', AVE: 'Ave', ST: 'St', STS: 'Sts', RD: 'Rd', BLVD: 'Blvd', BL: 'Bl', PL: 'Pl', PKWY: 'Pkwy', PKY: 'Pky',
  EXPWY: 'Expwy', EXPY: 'Expy', HWY: 'Hwy', DR: 'Dr', LN: 'Ln', TER: 'Ter', TERR: 'Terr', SQ: 'Sq', CT: 'Ct',
  TPKE: 'Tpke', TPK: 'Tpk', CIR: 'Cir', CTR: 'Ctr', BR: 'Br', HTS: 'Hts', JCT: 'Jct', STA: 'Sta', MT: 'Mt',
  FT: 'Ft', PT: 'Pt', BCH: 'Bch', BLDG: 'Bldg', HOSP: 'Hosp', CTRL: 'Ctrl', PLZ: 'Plz', PLAZ: 'Plaz', GDNS: 'Gdns',
  MNR: 'Mnr', LNDG: 'Lndg', XING: 'Xing', ISL: 'Isl', VLG: 'Vlg', HL: 'Hl', TRL: 'Trl', RDG: 'Rdg', VW: 'Vw',
};
const SMALL = new Set(['OF', 'AT', 'THE', 'AND', 'VIA', 'TO', 'ON', 'BY', 'IN', 'FOR', 'DE', 'DEL', 'LA', 'DI']);
...
// shouting = no lowercase letter outside the connectives the MTA writes in
// lower case even in its capitals ("KINGS PLAZA via UTICA")
const shouts = (n) => n.replace(/\b(via|to|at|of|and|the)\b/g, '') === n.replace(/\b(via|to|at|of|and|the)\b/g, '').toUpperCase();
const usName = (n) => {
  n = n.replace(/\s+/g, ' ').trim();
  if (!shouts(n)) return n; // already mixed case
  ...
  return parts.map((p) => {
    ...
    // "38 TH ST" → "38th St" (only Th and Nd are safe: "38 St", "38 Rd" and
    // "38 Av" are the numbered streets of Queens)
    .replace(/(\d+) (Th|Nd)\b/g, (m, a, b) => a + b.toLowerCase());
};
const njName = (n) => usName(n.replace(/-Exact Fare$/i, '').replace(/^\d+[A-Z]*\s+(?=[A-Z])/, ''));
const plainName = (n) => n.replace(/\s+/g, ' ').trim();

const boroughFeed = (tag) => ({
  tag, dir: `data/gtfs-${tag}`, routeTypes: ['3'], mapKey: (sn) => sn || null, nameFix: usName,
});
...
if (tramAll || lrtSel.length) MODES.push({
  // Hudson-Bergen Light Rail and the Newark Light Rail on railway=light_rail
  // (the Newark City Subway tunnel included). allVariants: the HBLR is ONE
  // route with three services — Tonnelle, West Side and 8th Street — so the
  // branches live only in the stop sequences.
  mode: 'tram', label: 'light rail', osmFile: 'data/osm/nyc-rail.json',
  graphMode: 'tram', railKeep: new Set(['light_rail', 'tram']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : lrtSel,
  feeds: [
    { tag: 'njr', dir: 'data/gtfs-njrail', routeTypes: ['0'], allVariants: true,
      skipRoute: (r) => !LRT_KEYS.has((r.route_short_name || '').trim()),
      // "West Side Avenue Light Rail Station" is a disc on a light rail line:
      // the sign says West Side Avenue
      mapKey: (sn) => sn || null, nameFix: (n) => usName(n).replace(/\s+(Light Rail (Station|Sta)|HBLR Station)$/i, ''), noFeedColors: true },
  ],
});
if (tramAll || subwaySel.length) MODES.push({
  // the subway, PATH and the SIR on railway=subway. allVariants on the subway: the
  // branches ARE the network (the A alone has three ends), and the feed's
  // platform-level stop ids are per direction, so the fingerprints stay
  // consistent within a direction and short-turns fold into the full runs.
  mode: 'tram', label: 'subway & PATH', osmFile: 'data/osm/nyc-rail.json',
  graphMode: 'tram', railKeep: new Set(['subway']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : subwaySel,
  feeds: [
    { tag: 'sub', dir: 'data/gtfs-subway', routeTypes: ['1'], allVariants: true,
      mapKey: (sn, r) => r.route_id || null, lineColor: (k) => NYC_COLORS[k], nameFix: plainName },
    { tag: 'path', dir: 'data/gtfs-path', routeTypes: ['2'],
      mapKey: (sn, r) => PATH_KEYS[r.route_id] || null, lineColor: (k) => NYC_COLORS[k], nameFix: plainName },
    // the Staten Island Railway is route_type 2 in the subway feed and
    // railway=subway in OSM — it rides this cfg, not the commuter rail one
    { tag: 'sir', dir: 'data/gtfs-subway', routeTypes: ['2'],
      mapKey: () => 'SIR', lineColor: (k) => NYC_COLORS[k], nameFix: plainName },
  ],
});
if (tramAll || railSel.length) MODES.push({
  // LIRR and Metro-North on railway=rail; allMetro
  // gives every line the trunk treatment. No allVariants: every branch is a
  // route of its own in both feeds, so the representative rule draws each
  // one whole.
  mode: 'tram', label: 'commuter rail', osmFile: 'data/osm/nyc-rail.json',
  graphMode: 'tram', railKeep: new Set(['rail']),
  allMetro: true,
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : railSel,
  feeds: [
    { tag: 'lirr', dir: 'data/gtfs-lirr', routeTypes: ['2'],
      mapKey: (sn, r) => lirrKey(r), lineColor: (k) => NYC_COLORS[k], nameFix: plainName },
    { tag: 'mnr', dir: 'data/gtfs-mnr', routeTypes: ['2'],
      mapKey: (sn, r) => (r.route_long_name || '').trim() || null, lineColor: (k) => NYC_COLORS[k], nameFix: plainName },
  ],
});
